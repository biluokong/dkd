  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="partnerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="250">
        <template #default="scope">
          <el-button link type="primary" @click="handleResetPwd(scope.row)" v-hasPermi="['manage:partner:edit']">重置密码</el-button>
          <el-button link type="primary" @click="handleShowInfo(scope.row)" v-hasPermi="['manage:partner:query']">查看详情</el-button>
    ...
    <el-dialog title="合作商详情" v-model="infoOpen" width="400px" append-to-body>
      <el-form ref="partnerRef" :model="form" label-width="100px">
        <el-form-item label="合作商名称：" prop="partnerName">{{ form.partnerName }}</el-form-item>
        <el-form-item label="联系人：" prop="contactPerson">{{ form.contactPerson }}</el-form-item>
        <el-form-item label="联系电话：" prop="contactPhone">{{ form.contactPhone }}</el-form-item>
        <el-form-item label="创建时间：" prop="createTime">{{ form.createTime }}</el-form-item>
        <el-form-item label="分成比例：" prop="profitRatio">{{ form.profitRatio }}</el-form-item>
        <el-form-item label="账号：" prop="account">{{ form.account }}</el-form-item>
      </el-form>
    </el-dialog>
  </div>
</template>
...
import { addPartner, delPartner, getPartner, listPartner, updatePartner, resetPwd } from '@/api/manage/partner'
...
const handleResetPwd = (row) => {
  const _ids = row.id || ids.value;
  proxy.$modal.confirm('是否确认重置合作商管理编号为"' + _ids + '"的密码？').then(() => {
    return resetPwd(_ids);
  }).then(() => {
    getList();
